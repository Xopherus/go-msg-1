	...
	for {
		...
		default:
			...
			if len(messages) == 0 {
				...
				// TODO - there isn't a concept of message retries w/ DLQ. A message could fail to be processed indefinitely.
				// We could recreate this by using XCLAIM+XPENDING separately (the latter will return idle time + retries)
			}
